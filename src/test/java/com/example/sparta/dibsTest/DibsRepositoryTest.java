...
public class DibsRepositoryTest {
  ...
  void test1(){
    ...
    //when
    //create dibs
    Store findStore = storeRepository.findById(id).orElseThrow(()->new NoSuchElementException("해당 가계는 찾을수 없어요"));
    ...
    Dibs removedibs =dibsRepository.findDibsByStoreAndUser(store,user).orElseThrow(()->new NoSuchElementException("해당 가계는 찾을수 없어요"));
